...
class MobileDbTableData extends DataClass
    implements Insertable<MobileDbTableData> {
  final int id;
  final String name;
  final bool isEnabled;
  MobileDbTableData(
      {@required this.id, @required this.name, @required this.isEnabled});
  factory MobileDbTableData.fromData(
      Map<String, dynamic> data, GeneratedDatabase db,
      {String prefix}) {
    final effectivePrefix = prefix ?? '';
    final intType = db.typeSystem.forDartType<int>();
  ...
  factory MobileDbTableData.fromJson(Map<String, dynamic> json,
      {ValueSerializer serializer = const ValueSerializer.defaults()}) {
    return MobileDbTableData(
      id: serializer.fromJson<int>(json['id']),
      name: serializer.fromJson<String>(json['name']),
  ...
  @override
  MobileDbTableCompanion createCompanion(bool nullToAbsent) {
    return MobileDbTableCompanion(
      id: id == null && nullToAbsent ? const Value.absent() : Value(id),
      name: name == null && nullToAbsent ? const Value.absent() : Value(name),
      isEnabled: isEnabled == null && nullToAbsent
          ? const Value.absent()
  ...
  MobileDbTableData copyWith({int id, String name, bool isEnabled}) =>
      MobileDbTableData(
        id: id ?? this.id,
        name: name ?? this.name,
        isEnabled: isEnabled ?? this.isEnabled,
  ...
  @override
  String toString() {
    return (StringBuffer('MobileDbTableData(')
          ..write('id: $id, ')
          ..write('name: $name, ')
          ..write('isEnabled: $isEnabled')
          ..write(')'))
  ...
  @override
  int get hashCode =>
      $mrjf($mrjc(id.hashCode, $mrjc(name.hashCode, isEnabled.hashCode)));
  @override
  bool operator ==(dynamic other) =>
      identical(this, other) ||
      (other is MobileDbTableData &&
          other.id == this.id &&
          other.name == this.name &&
          other.isEnabled == this.isEnabled);
}

class MobileDbTableCompanion extends UpdateCompanion<MobileDbTableData> {
  final Value<int> id;
  final Value<String> name;
  final Value<bool> isEnabled;
  const MobileDbTableCompanion({
    this.id = const Value.absent(),
    this.name = const Value.absent(),
    this.isEnabled = const Value.absent(),
  });
  MobileDbTableCompanion.insert({
    this.id = const Value.absent(),
    @required String name,
    this.isEnabled = const Value.absent(),
  }) : name = Value(name);
  MobileDbTableCompanion copyWith(
      {Value<int> id, Value<String> name, Value<bool> isEnabled}) {
    return MobileDbTableCompanion(
      id: id ?? this.id,
      name: name ?? this.name,
      isEnabled: isEnabled ?? this.isEnabled,
...
class $MobileDbTableTable extends MobileDbTable
    with TableInfo<$MobileDbTableTable, MobileDbTableData> {
  final GeneratedDatabase _db;
  final String _alias;
  $MobileDbTableTable(this._db, [this._alias]);
  final VerificationMeta _idMeta = const VerificationMeta('id');
  GeneratedIntColumn _id;
  @override
  GeneratedIntColumn get id => _id ??= _constructId();
  ...
  @override
  List<GeneratedColumn> get $columns => [id, name, isEnabled];
  @override
  $MobileDbTableTable get asDslTable => this;
  @override
  String get $tableName => _alias ?? 'mobile_db_table';
  @override
  final String actualTableName = 'mobile_db_table';
  @override
  VerificationContext validateIntegrity(MobileDbTableCompanion d,
      {bool isInserting = false}) {
    final context = VerificationContext();
    if (d.id.present) {
      context.handle(_idMeta, id.isAcceptableValue(d.id.value, _idMeta));
  ...
  Set<GeneratedColumn> get $primaryKey => {id};
  @override
  MobileDbTableData map(Map<String, dynamic> data, {String tablePrefix}) {
    final effectivePrefix = tablePrefix != null ? '$tablePrefix.' : null;
    return MobileDbTableData.fromData(data, _db, prefix: effectivePrefix);
  }

  @override
  Map<String, Variable> entityToSql(MobileDbTableCompanion d) {
    final map = <String, Variable>{};
    if (d.id.present) {
      map['id'] = Variable<int, IntType>(d.id.value);
    }
  ...
  @override
  $MobileDbTableTable createAlias(String alias) {
    return $MobileDbTableTable(_db, alias);
  }
}
...
abstract class _$AppDatabase extends GeneratedDatabase {
  ...
  $MobileDbTableTable _mobileDbTable;
  $MobileDbTableTable get mobileDbTable =>
      _mobileDbTable ??= $MobileDbTableTable(this);
  PackageTableDao _packageTableDao;
  PackageTableDao get packageTableDao =>
      _packageTableDao ??= PackageTableDao(this as AppDatabase);
  MobileDbTableDao _mobileDbTableDao;
  MobileDbTableDao get mobileDbTableDao =>
      _mobileDbTableDao ??= MobileDbTableDao(this as AppDatabase);
  @override
  List<TableInfo> get allTables => [packageTable, mobileDbTable];
}
...
mixin _$MobileDbTableDaoMixin on DatabaseAccessor<AppDatabase> {
  $MobileDbTableTable get mobileDbTable => db.mobileDbTable;
}
